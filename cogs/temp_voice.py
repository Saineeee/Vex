...
class TempVoice(commands.Cog):
    def __init__(self, bot):
        ...
        self.temp_vcs = bot.db["temp_vcs"]
        # In-memory cache for settings to prevent DB queries on every voice state update
    ...
    async def setuptempvc(self, interaction: discord.Interaction):
        ...
        # Update cache
        if not self._settings_cache.get(interaction.guild.id):
            existing = await self.settings.find_one({"guild_id": interaction.guild.id})
            self._settings_cache[interaction.guild.id] = existing or {"guild_id": interaction.guild.id}
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if member.guild.id not in self._settings_cache:
            self._settings_cache[member.guild.id] = await self.settings.find_one({"guild_id": member.guild.id})

        settings = self._settings_cache.get(member.guild.id)
