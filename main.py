...
async def api_update_settings(settings: SettingsUpdate, authorization: str = Header(None)):
    ...
    # Verify authorization with Discord API
    async with httpx.AsyncClient() as client:
        guilds_res = await client.get("https://discord.com/api/users/@me/guilds", headers={"Authorization": f"Bearer {access_token}"})
        ...
        has_admin = False
        for g in guilds:
            if int(g["id"]) == settings.guild_id:
                if (int(g["permissions"]) & 0x8) == 0x8:
                    has_admin = True
                break

        if not has_admin:
            raise HTTPException(status_code=403, detail="Forbidden: You do not have Administrator permissions in this server.")
