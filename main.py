...
async def api_update_settings(settings: SettingsUpdate, authorization: str = Header(None)):
    ...
    async with httpx.AsyncClient() as client:
        # Verify the token and check admin permissions for the guild
        guilds_res = await client.get("https://discord.com/api/users/@me/guilds", headers={"Authorization": f"Bearer {access_token}"})
        ...
        is_admin = False
        for g in guilds:
            if g["id"] == str(settings.guild_id):
                if (int(g["permissions"]) & 0x8) == 0x8:
                    is_admin = True
                break

        if not is_admin:
            raise HTTPException(status_code=403, detail="Not authorized to configure this server.")
