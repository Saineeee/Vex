...
async def api_update_settings(settings: SettingsUpdate, request: Request):
    ...
    async with httpx.AsyncClient() as client:
        guilds_res = await client.get("https://discord.com/api/users/@me/guilds", headers={"Authorization": f"Bearer {token}"})
        if guilds_res.status_code != 200:
            raise HTTPException(status_code=401, detail="Invalid token")

        has_admin = any(int(g["id"]) == settings.guild_id and (int(g.get("permissions", 0)) & 0x8) == 0x8 for g in guilds_res.json())
        if not has_admin:
