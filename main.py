...
async def api_update_settings(settings: SettingsUpdate, request: Request):
    ...
    # Verify token and user permissions via Discord API
    res = await bot.http_session.get("https://discord.com/api/users/@me/guilds", headers={"Authorization": f"Bearer {access_token}"})
    if res.status_code != 200:
        raise HTTPException(status_code=401, detail="Invalid token.")

    guilds = res.json()
    is_admin = any(g["id"] == str(settings.guild_id) and (int(g["permissions"]) & 0x8) == 0x8 for g in guilds)

    if not is_admin:
        raise HTTPException(status_code=403, detail="Missing Administrator permissions in this server.")
